...
def call_naver_shopping(api_id, api_secret, keyword):
    url = f"https://openapi.naver.com/v1/search/shop.json?query={keyword}&display=5" # 상위 5개만
...
def get_product_deep_dive_report(model, product_name, internal_rank, datalab_result, shopping_result):
    ...
    **나. 외부 시장 현황 (External Market - Source: Naver API)**
    - **검색량 트렌드 (최근 1년):** {json.dumps(datalab_result['results'][0]['data'], ensure_ascii=False)}
    ...
    if uploaded_file:
        with st.spinner("데이터를 처리하는 중입니다..."):
            try:
                ...
                # 제외 로직 강화: .str.strip()으로 확실하게 공백 제거 후 비교
...
if st.session_state.analysis_df is None:
    ...
    with tab3:
        ...
        product_list = analysis_df['제품명'].unique()
            ...
            if not all([g_model, n_id, n_secret]):
                ...
                with st.spinner(f"'{selected_product}'에 대한 내/외부 데이터를 종합 분석 중입니다..."):
                    ...
                    if datalab_result and shopping_result:
                        ...
                        st.error("외부 시장 데이터를 가져오는 데 실패했습니다.")
